...
function save(self,type,action,itemid){
    if(type=="name"){
        ...
        $.ajax({
                type: "POST",
                url: '<?php echo base_url('admin/ajax/editname');?>',
    ...
    if(type=="action"){
        if (action == "edit") {
            ...
            $.ajax({
                type: "POST",
                url: '<?php echo base_url('admin/ajax/editaction');?>',
        ...
        if (action == "add") {
            ...
            $.ajax({
                type: "POST",
                url: '<?php echo base_url('admin/ajax/addmenu');?>',
                data: postStr,
                dataType: 'json',
                success: function(data){
                    if (data.status == 200) {
...
function delitem(itemid){
    ...
    //
    $.getJSON("<?php echo base_url('admin/ajax/delmenu/');?>"+"/"+itemid,function(result){
...
function fetchmenu(itemid){
    var wid = $('.flag').html();
    $.getJSON("<?php echo base_url('admin/wechat/fetch/');?>"+"/"+wid,function(result){
        console.log(result);
...
function initmenu(){
    ...
    $.ajax({
        type: "POST",
        url: '<?php echo base_url('admin/ajax/initmenu');?>',
        data: "wid="+wid,
        dataType: 'json',
        success: function(data){
            if (data.status == 200) {
...
function pushmenu(){
    ...
    $.ajax({
        type: "POST",
        url: '<?php echo base_url('admin/ajax/pushmenu');?>',
        data: "wid="+wid,
        dataType: 'json',
        success: function(data){
            if (data.errcode == 0) {
                alert("自定义菜单推送成功");
            }else if(data.errcode == 40018 || data.errcode == 40016){
                alert("自定义菜单格式有误，推送失败");
            }else if(data.errcode == 40033){
                alert("自定义菜单格式有误，请检查你设置的响应事件");
            }
        },
        error: function() {
            alert("推送过程中发生异常，失败");
...
function delRemoteMenu(){
    ...
    $.ajax({
        type: "POST",
        url: '<?php echo base_url('admin/ajax/delRemoteMenu');?>',
        data: "wid="+wid,
        dataType: 'json',
        success: function(data){
            if (data.errcode == 0) {
                alert("已成功删除微信服务器上的自定义菜单");
            }else if(data.errcode == 40001 || data.errcode == 40016){
                alert("AccessToken错误，请再次尝试");
            }
        },
        error: function() {
            alert("提交过程中发生异常，失败");
