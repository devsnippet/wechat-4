...
class Wechat
{
	...
	public function __construct($options)
	{
        ...
        //TODO load model
        $this->CI->load->model('wechat_setting_model', 'setting');
...
<Nonce><![CDATA[%s]]></Nonce>
	 ...
	 * @return boolean
	 */
	protected function setCache($type, $cachename, $value, $expired){
		return $this->CI->setting->cache_set($type, $cachename, $value, $expired);
	}
	 ...
	 * @return mixed
	 */
	protected function getCache($type, $cachename, $kind = "token"){
		return $this->CI->setting->cache_get($type, $cachename, $kind);
	}
	 ...
	 * @return boolean
	 */
	protected function removeCache($type, $cachename){
		//TODO: remove cache implementation
		return $this->ci->setting->cache_remove($type, $cachename);
	}
	...
	public function checkAuth($appid='',$appsecret='',$token=''){
        ...
        $authname = $appid;
        //Modfied by yuqi.zhou
        //检查是否过期 如果没过期直接从数据库中取来用 
        if ($expired = $this->getCache("at", $authname, "expired")) {
            if (time() < $expired) {
                if ($rs = $this->getCache("at", $authname))  {
		...
		if ($result)
		{
            ...
            $expired = time()+$expire;
			$this->setCache("at", $authname, $this->access_token,$expired);
			return $this->access_token;
		}
		return false;
	}
	...
	public function getJsTicket($appid='',$jsapi_ticket=''){
		...
		$authname = $appid;
		if ($rs = $this->getCache("jt", $authname))  {
		...
		if ($result)
		{
			...
			$expire = $json['expires_in'] ? intval($json['expires_in'])-100 : 3600;
            $expired = time()+$expire;
			$this->setCache("jt", $authname,$this->jsapi_ticket,$expired);
			return $this->jsapi_ticket;
		}
		return false;
	}
